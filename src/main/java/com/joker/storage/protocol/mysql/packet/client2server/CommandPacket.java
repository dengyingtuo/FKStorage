...
public class CommandPacket extends BasePacket {
    ...
    public byte command;	/* 命令的类别 */
    public byte[] arg;

    public void read(ByteBuffer buffer) {
        super.read(buffer);
        command = BufferUtil.readByte(buffer);
        arg = BufferUtil.readRemainBytes(buffer);
    }
    ...
    public ByteBuffer write(ByteBuffer buffer, MysqlConnection c) {
        ...
        BufferUtil.writeUB1(buffer, command);
        BufferUtil.writeBinaryByte(buffer, arg);
        return buffer;
    }

    @Override
    public int calcPacketSize() {
        return super.calcPacketSize() + 1 + arg.length;
    }
